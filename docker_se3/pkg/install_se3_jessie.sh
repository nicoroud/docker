...
if [ ! "$rep" = "n" ]; then
	...
	echo -e "$COLCMD\c "
	mysqladmin -h $MYSQLIP password $MYSQLPW 2>/dev/null && echo -e "${COLINFO}Le mot de passe root MySQL a été initialisé à $MYSQLPW"
	...
	# Initialisation de la Base de Données
		...
		mysqladmin -h $MYSQLIP create se3db -u root -p$MYSQLPW
		...
		mysqladmin -h $MYSQLIP reload -u root -p$MYSQLPW
